...
ROOT.gROOT.ProcessLine("new TColor(124, 52/255.,  73/255.,  94/255.,\"wet-asphalt\"   );")
ROOT.gROOT.ProcessLine("new TColor(138,189/255., 195/255., 199/255.,\"silver\"        );")
ROOT.gROOT.ProcessLine("new TColor(139,127/255., 140/255., 141/255.,\"asbestos\"      );")
...
ROOT.gStyle.SetTitleXSize(label_size)
ROOT.gStyle.SetTitleYSize(label_size)
ROOT.gStyle.SetLegendTextSize(label_size)
...
parser.add_argument('-cfg', '--cfg', type=str, default="./config/Config_MC_2016.yml", help="")
#parser.add_argument('-cfg', '--cfg', type=str, default="config/new.yml", help="")
...
with open(options.cfg, 'r') as stream:
    ...
    processes = yaml.load(stream, Loader=yaml.FullLoader)
xsections = {}
...
def draw_cms_headlabel(label_left  ='#scale[1.2]{#bf{CMS}} #it{Preliminary}',
    ...
    tex_left.DrawLatexNDC (0.14,
                           1.03 - ROOT.gStyle.GetPadTopMargin(),label_left)
    tex_right.DrawLatexNDC(1-0.05,
                           1.03 - ROOT.gStyle.GetPadTopMargin(),label_right)
...
def drawing(channel="_3L",observable_="dijet_Mjj",names_="test",ranges_=[100,1500], ylog=True, lumi=41.5, blind=True, rebin=1):
    x_vec = collections.OrderedDict(processes)
    ...
    for procname, cmd  in x_vec.items():
        files     = cmd["files"]
        hist_objs = []
        dic_root_histos = {}
        for fn in files:
            ...
            for hist_name in hist_names:
                ...
                if cmd.get("type").lower() == "background":
                    ...
                    #                    scale *= lumi/bn_root.Get("genEventSumw").sum()
                    scale *=lumi/bn_root.Get("h_total").GetBinContent(1)
                    scale *= cmd.get("kfactor", 1.0)
                    hist.Sumw2()
                    hist.Scale(scale)
                    hist.SetDirectory(0)
                    print(fn, hist.Integral())
       ...
       # hist_com.SetTitle(";{};Events".format(names[channel.replace("_","")]))
        hist_com.SetTitle(";{};Events".format(names_))  
    ...
    t.DrawLatexNDC((0.07 + ROOT.gStyle.GetPadLeftMargin()),
    ...
    dirName="plots/PhotonCR/"

    if not os.path.exists(dirName+"{}".format(options.era)):
        os.makedirs(dirName+"{}".format(options.era))
    dirname__ = dirName+"{}".format(options.era, channel)    
    plotname__ = dirName+"{}/plot_{}_{}_region".format(options.era, observable_,channel)    
    c.SaveAs(plotname__+".pdf")
    c.SaveAs(plotname__+".png")
    outputname = 'cp  -r '+ dirname__ +' /eos/user/m/mmittal/www/VBSZZ2l2nu/'
    outputname1 = 'cp  -r  /eos/user/m/mmittal/www/index.php  /eos/user/m/mmittal/www/VBSZZ2l2nu/'
    outputname2 = 'cp  -r /eos/user/m/mmittal/www/index.php  /eos/user/m/mmittal/www/VBSZZ2l2nu/2016/'
#    outputname3 = 'cp  -r /eos/user/m/mmittal/www/index.php  /eos/user/m/mmittal/www/VBSZZ2l2nu/plots/2016/'

    
    os.system(outputname)
    os.system(outputname1)
    os.system(outputname2)
 #   os.system(outputname3)
...
#    drawing("CR" , lumi=lumi[options.era])
    


    drawing("CRDY" , observable_="dijet_Mjj",names_="M_{jj}(GeV)", ranges_=[100,1400],lumi=lumi[options.era])
    drawing("SR" , observable_="dijet_Mjj",names_="M_{jj}(GeV)", ranges_=[100,1500],lumi=lumi[options.era])
#    drawing("CR" , observable_="dijet_Mjj",names_="M_{jj}(GeV)", ranges_=[100,400],lumi=lumi[options.era])
